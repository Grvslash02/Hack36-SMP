...
exports.protect= async (req,res,next)=>{
    ...
    if(!freshUser){
        return res.status(401).json({ success: false, message: 'The user belonging to the token does not longer exists' });
    }   
...
exports.restrictTo = (...roles)=>{
    return (req,res,next)=>{
        ...
        if(!roles.includes(req.user.role)){
            return res.status(403).json({ success: false, message: 'DO you really think you have access to do this?' });
        }
...
exports.sendOTP = async(req, res) => {
    ...
    if(isVerified !== true){
        return res.status(400).json({ success: false, message: 'Verify Details First' });
    }
    if(!email){
        return res.status(400).json({ success: false, message: 'Email Address Not Known' });
    }
